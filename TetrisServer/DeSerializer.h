...
//6번 배열의 마지막 값은 uint64_max를 넣음으로서 LEN을 판별한다.


class DeSerilizer
{
public:
	static shared_ptr<vector<PacketVariant>> Deserialization(BYTE* pos, int len);

	static void ConvertMemory(BYTE* pos, void* data, size_t len, int currentByte);
};
